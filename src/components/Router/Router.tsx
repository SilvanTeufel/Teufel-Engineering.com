...
class Router extends React.Component<{}, MyComponentState> {
  ...
  createScrollButton = (key0: string) => {
    const button = [];
    button.push(
      <Link to={key0} spy={true} smooth={true} offset={-120} duration={500} delay={200}>
        <button type="button" className="btn btn-outline-light" style={{ minWidth: 110 }} onClick={() => this.redirectToLink(key0, 'General')}>
          {key0}
        </button>
        </Link>
    ...
    if (this.state && this.state.routerstructure){
      ...
      <div className="d-none d-md-block">
          <NavbarCollapse Key={key0} getIconState={this.getIconState} buttons={this.createSubButtonArray(key0)} />
      </div>
      )
    }
  ...
  createSubButton = (key0: string, key1: string) => {
    ...
    button.push(
      <Link to={key0} spy={true} smooth={true} offset={-120} duration={500} delay={200}>
        <button type="button" className="btn btn-outline-dark" style={{ width: 90, maxHeight: 32 }} onClick={() => this.redirectToLink(key0, key1)}>
          {key1}
        </button>
      </Link >
    );
  ...
  createComponents = () => {
    ...
    for (var key0 in this.state.routerstructure.navKeys)
    {
        for (var key1 in this.state.routerstructure.navKeys[key0]) {
          comps.push(
            <CustomRoute path={`/${key0}/${key1}`} component={Home} />
          );
        }
    }

    comps.push(
      <CustomRoute path="/Datenschutz" component={Datenschutz} />
    );
    comps.push(
      <CustomRoute path="/Impressum" component={Impressum} />
    );
    comps.push(
      ...
      <HashRouter>
        ...
        <div>
          <Helmet>
            ...
            <meta name="description" content={'Teufel Engineering - Silvan Teufel - Master of Science'} />
          </Helmet>
          {this.createHeader()}
